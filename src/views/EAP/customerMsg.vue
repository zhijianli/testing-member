                ...
                <div class="widget-content nopadding">
                    <div class="form-horizontal">
                        ...
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <input type="text" v-model="info.consumedOrderAmount" disabled> 单
                            </div>
                        </div>
                        <!-- 新增不显示 编辑禁止改 -->
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <input type="text" v-model="info.consumedDuration | secToMin" disabled> 分钟
                            </div>
                        </div>
                        <div class="control-group">
                            <label class="control-label">购买时段 :</label>
                            <div class="controls">
                                <input class="input" size="30" type="text" @click.stop="open($event,'picker3')" v-model="calendar.items.picker3.value" placeholder="开始时间 此项必填"> 至
                                ...
                                <calendar :show.sync="calendar.show" :type="calendar.type" :value.sync="calendar.value" :x="calendar.x" :y="calendar.y" :begin.sync="calendar.begin" :end.sync="calendar.end" :range.sync="calendar.range" :weeks="calendar.weeks" :months="calendar.months">
                                </calendar>
                            </div>
                        </div>
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <input type="text" v-model="info.remainingDays" v-bddisabled="info.eeId"> 天
                            </div>
                        </div>
                        <!--<div class="control-group">-->
                        ...
                        <!--</div>-->
                        <div class="control-group">
                            <label class="control-label"> 对外开放 :</label>
                            <div class="controls">
                                <select name="" id="select" v-model='info.isOpened' style="width: 180px;">
                        ...
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <textarea name="" id="" cols="100" rows="5" v-model="info.notice"></textarea>
                            </div>
                        </div>
                        <div class="control-group">
...
//                pgId: this.info.pgId, //    服务心理医师组id   number
                ...
                isOpened: this.info.isOpened // 对外开放

            }

            paramsObj.eeId = this.info.eeId ? this.info.eeId : ''

            let params = serialize(paramsObj)

            let data = ''
                if (this.info.eeId) {
                    data = await EAPupdateEnterprise(params)
                } else {
                    data = await EAPnewEnterprise(params)
                }
            if (data.code === 0) {
                this.flag=true;
    ...
    directives: {
        'bddisabled': {
            update(ID) {
                this.el.disabled = ID == undefined ? true : false
            }
        }
    },
    ready() {
//        this.getPgid();
        // !!this.$route.params.id ? this.getData() : console.log('新增客户')
