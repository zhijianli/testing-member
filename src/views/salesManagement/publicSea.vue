        ...
        <table class="table table-bordered table-striped with-check" id="DataTables_Table_0">
            ...
            <tr v-for="item in list">
                ...
                <td class="userList_opeartion">
                    ...
                    <span class="crActive" @click="crowd(item.id)" v-if="n21">放入废弃池</span>
                </td>
            </tr>
            </tbody>
        </table>
...
//                let data = await getCourse("curriculum/selectCurriculum.json", `pageIndex=${this.page}&pageSize=${this.pageSize}`);
                let data = await getCourse("crm/clue/getInternationalWatersClues", params);
...
//                let data = await getCourse("curriculum/selectCurriculum.json", `pageIndex=${this.page}&pageSize=${this.pageSize}`);
                let data = await getCourse("crm/clue/fromInternationalWaterToAbandonedPool", `crmClueId=${params}`);
