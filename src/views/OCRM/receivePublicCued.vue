...
<template>
    ...
    <div class="container-fluid">
        <div id="search" class="title" style="margin-top: 40px">
            <ul class="power">
                <li @click="crowdAll">领取已选择的线索</li>
            </ul>
            客户来源：
        ...
        <table class="table table-bordered table-striped with-check" id="DataTables_Table_0">
            ...
            <tr v-for="item in list">
                ...
                <td class="userList_opeartion">
                    <span class="crActive" @click="jump(item.id)">客户信息</span>
                    <span class="crActive" @click="crowd(item.id)">领取</span>
...
<script type="text/javascript">
    ...
    export default {
        ...
        methods: {
            ...
            crowd(id){
                let paramsObj = {};
//                console.log(JSON.stringify(this.selectPersonList))

                paramsObj.crmClueIds=`[${id}]`
                paramsObj.userId=sessionStorage.userId
...
//                console.log(JSON.stringify(this.selectPersonList))
                ...
                if(!this.selectPersonList[0]){
                    alert('请您选择线索')
                }else{
                    paramsObj.crmClueIds=JSON.stringify(this.selectPersonList)
                    this.receive(this.serialize(paramsObj))
                }
...
//                let data = await getCourse("curriculum/selectCurriculum.json", `pageIndex=${this.page}&pageSize=${this.pageSize}`);
                let data = await getCourse("crm/clue/getReceiveInternationalWatersClues", params);
