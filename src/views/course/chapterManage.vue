    ...
    <div class="container-fluid">
        ...
        <div id="search" class="title">
            <button class="btn btn-success" style="margin:0 0 10px;"><a href="javascript:;"
                                                                        @click="changeRoute('courseManage')"
                                                                        style="color:white;">返回</a></button>
            <button class="btn btn-success" style="margin:0 0 10px;"><a href="javascript:;"
                                                                        @click="newAdd(courseId)"
        ...
        <table class="table table-bordered table-striped" id="DataTables_Table_0">
            ...
            <tr v-for="item in chapter">
                ...
                <td>
                    <template v-if="item.backPic">
                        <img :src=" imgPre + '/' + item.backPic" alt="">
                    </template>
                </td>
                <td>{{ item.hourLong }}</td>
                <td>{{ item.createTime }}</td>
                <td>{{ item.studyNums }}</td>
                <td>{{item.studyEndNums}}</td>
                <td v-text="item.chapterStatus?'下架':'上架'"></td>


                <td v-text="item.auditionStatus?'非试听':'试听'"></td>
                <td class="userList_opeartion">
                    <span @click="jump(item.id,courseId)">编辑</span>
                    <span @click="enable(item.id,item.chapterStatus)" v-text="item.chapterStatus?'上架':'下架'"></span>
                    <span @click="enaudition(item.id,item.auditionStatus)" v-text="item.auditionStatus?'试听':'非试听'"></span>
...
<script type="text/javascript">
    ...
    export default {
        ...
        methods: {
            newAdd(id){
                this.$dispatch('child-msg', "新建/修改章节详情")
                this.$route.router.go(`/newChapter/${id}`)
            },
            jump(id,cid) {
                this.$dispatch('child-msg', "新建/修改章节详情")
                this.$route.router.go(`/newChapter/${cid}/${id}`)
            },
            ...
            async getScale() {
                let data = await getCourse("curriculumChapter/selectChapter.json", `curriclumId=${this.courseId}&pageIndex=${this.page}&pageSize=${this.pageSize}`);
            ...
            async isEnable(id, num) {
                let data = await getCourse("curriculumChapter/updateChapterStatus.json", `id=${id}&chapterStatus=${num}`);
            ...
            async isAudition(id, num) {
                let data = await getCourse("curriculumChapter/updateChapterStatus.json", `id=${id}&auditionStatus=${num}`);
            ...
            async deleteScale(id) {
                let data = await getCourse("curriculumChapter/deleteChapter.json", `id=${id}`);
                console.log(data);
                if (data.code == 0) {
                    this.getScale();
                } else {
