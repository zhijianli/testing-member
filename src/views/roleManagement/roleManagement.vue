...
<template>
    ...
    <div class="container-fluid">
        <div id="search" class="title" style="margin-top: 30px">
            ...
            <lable>
                角色名称：
                <input type="text" name="curriculumTitle" v-model="words">
            </lable>
        ...
        <button class="btn btn-success" style="margin-right:30px;margin-bottom: 20px;"><a href="javascript:;"
                                                                      @click="changeRoute('newRole')"
        ...
        <table class="table table-bordered table-striped" id="DataTables_Table_0">
            ...
            <tr v-for="item in list">
                ...
                <td class="userList_opeartion">
                    <span class="crActive" v-if="item.roleTypeStatus" @click="jump(item.roleId)">编辑</span>
                    <!--<span class="crActive" v-if="item.roleTypeStatus" @click="delete(item.roleId)">删除</span>-->
    ...
    export default {
        ...
        methods: {
            ...
            changeRoute(url) {
                ...
                if (url == "newRole") {
                    this.$dispatch('child-msg', "新建/修改角色")
                }
            ...
            jump(id) {
                this.$dispatch('child-msg', "编辑角色")
                this.$route.router.go(`/newRole?id=${id}`)
            },
...
//                let data = await getCourse("curriculum/selectCurriculum.json", `pageIndex=${this.page}&pageSize=${this.pageSize}`);
                let data = await getCourse("systemRole/roleList.json", params);
                if (data.code === 0) {
                    this.list = data.list;
            ...
            async delete(a) {
                let data = await getCourse("systemRole/deleteRole.json",`roleId=${a}`);
