...
<template>
    <div style="overflow-x:auto ">
        <div style="min-width: 130%;background: #f5f5f5">
            <div class="container-fluid">
                <div id="search" class="title" style="width: 1616px">
                    <span>手机号：</span>
                    <input type="text" style="width: 175px;" name="words" v-model="mobilePhone">
                    <span>微信号：</span>
                    <input type="text" style="width: 175px;" name="words" v-model="wxAccount">
                    <span style="margin-left: 20px;">昵称：</span>
                    <input type="text" style="width: 170px;" name="words" v-model="nickname">
                    <span style="margin-left: 6px;">性别：</span>
                    <select v-model="sex" name="regTimeDirection">
                    ...
                    <br >
                    <span style="margin-left: 15px;width: 193px;">禁用：</span>
                    <select v-model="isDisable">
                        <option value="0">全部</option>
                        <option value="1">已禁用</option>
                        <option value="2">正常</option>
                    </select>
                    <span style="margin-left: 9px;">禁言：</span>
                    <select v-model="isGag">
                        <option value="0">全部</option>
                    ...
                    <span>排序按：</span>
                    <select v-model="sortType" style="width: 190px;">
                        <option value="2">全部</option>
                        <option value="0">注册时间逆序</option>
                        <option value="1">最近看过专家详情页的时间</option>
                    </select>
                    <span>其他：</span>
                    <select v-model="other" name="regTimeDirection">
                        <option value="0">全部</option>
                        <option value="1">Ping++记录的未支付时间在30天内的</option>
                        <!--<option value="2">30天内发生过私聊的</option>-->
                    </select>
                    <span style="margin-left: 28px;">年龄：</span>
                    <input type="text" style="width: 161px;margin-right: 3px;" style="width: 53px" v-model="ageStart">-
                    <input type="text" style="width: 160px;" style="width: 53px" v-model="ageEnd">

                    <div style="display: inline-block">
                      <span>跟进人：</span>
                      <input type="text" style="width: 175px;" name="words" v-model="genjinPeople">

                    <span>注册渠道：</span>
                    <select v-model="channel1"  @change="channelChangeFn" style="width: 181px;">
                        <option value="0">全部</option>
                        <option :value="item.primaryChannelName" v-for="item in channelList">{{item.primaryChannelName}}</option>
                    </select>
                    <select v-model="channel2">
                        <option value="0">全部</option>
                        <option :value="item" v-text="item" v-for="item in channel2List" :key="item"></option>

                    </select>
                    累计消费金额在 <input type="text" style="width: 175px;" style="width: 50px" v-model="amount">元以上
                ...
                <table class="table table-bordered table-striped with-check" id="DataTables_Table_0">
                    ...
                    <tr>
                        ...
                        <th style="width: 80px">注册渠道一级</th>
                        <th style="width: 80px">注册渠道二级</th>
                        <th style="width: 80px">累计充值金额（元）</th>
                        <th style="width: 80px">累计消费金额</th>
                        <th style="width: 80px">近30天内累计消费金额</th>
                        <th style="width: 80px">已完成订单数</th>
                        <th style="width: 80px">投诉/退款（处理中+已处理）订单数</th>
                        <th style="width: 80px">已取消订单数</th>
                        <th style="width: 80px">Ping++记录的最近的未支付时间</th>
                        <th style="width: 80px">最近看过的5个专家详情页的专家手机号</th>
                        <th style="width: 80px">最近看过专家详情页的时间</th>
                        <!--<th style="width: 80px">最近的私聊时间</th>-->
                        <th style="width: 50px">提问数</th>
                        <th>购买锦旗数</th>
                        <!--<th style="width: 50px">完成心理测试数</th>-->
                        <th style="width: 50px">圈子内容发布数</th>
                        <th style="width: 50px">圈子评论数</th>
                        <th style="width: 50px">跟进人</th>
                        <th style="width: 200px">备注</th>
                        <th style="width: 35px">禁用状态</th>
                        <th style="width: 35px">禁言状态</th>
                    ...
                    <tr v-for="item of list">
                        ...
                        <td>{{ item.nickname }}</td>
                        <td v-text=" item.sex | sex"></td>
                        <td v-text=" item.birthday | birthday"></td>
                        <td>{{ item.city }}</td>
                        <td>{{ item.profession }}</td>
                        <td>{{ item.regTime }}</td>
                        <td>{{ item.primaryChannelName }}</td>
                        <td>{{ item.twoLevelChannelName }}</td>
                        ...
                        <td style="word-wrap: break-word;max-width: 200px">{{ item.remarks }}</td>
                        <td>{{ item.isDisable|editIsEnableFilter}}</td>
                        <td>{{ item.isGag|isEnableFilter}}</td>
                        <td>
                            <span class="normal-operate-state" @click="editRemarkFn(item.mid)" style="min-width: 0">编辑</span>
                            <a href="#" class="operation editArticle normal-operate-state" style="min-width: 0" @click="privateLetter(item.mid,item.responsible,item.imAccount)" v-show="personal&&item.imAccount">私信</a>
                        </td>
                    </tr>
                    </tbody>
                </table>
    ...
    export default {
        ...
        data() {
            return {
                ...
                caid:"",
                personal:false,
                responsible:sessionStorage.username,
...
//            私信
            async privateLetter(mid,responsible,imAccount){
                if(!responsible){
                    this.addResponsible(mid ,responsible)
                }else{
                    this.ChatPage(mid,imAccount)
                }


            },
//            添加跟进人
            async addResponsible(mid ,responsible){
                let obj = {
                    mid: mid,
                    responsible:this.responsible
                }
                let data = await getResponses('depression/member/updateById.json',this.serialize(obj));
                if(data.code===0){
                    this.ChatPage(mid)
                }
            },
//            主动发起
            async ChatPage(mid,imAccount){
                let obj = {
                    mid: mid,
                    type:2
                }
                let data = await getResponses('assistant/updateMemberIM.json',this.serialize(obj),"/depression-api/");
                if(data.code===0){
//                    if(nim){
...
//                        }
//                    }
                    this.sendMessage(mid,imAccount)
                }
            },
//            发初始消息
            async sendMessage(mid,imAccount){
                let obj = {
                    mid: mid,
                    caid:this.caid
                }
                let data = await getResponses('assistant/sentInitialImMsg.json',this.serialize(obj),"/depression-api/");
                if(data.code===0){
                    this.$route.router.go(`/imChat?id=${imAccount}`)
                }
            },
            ddd(start, end) {
            ...
            paramsFn() {
                ...
                this.start && (params.regTimeStart = this.start);
                this.end && (params.regTimeEnd = this.end);
                this.amount.trim() && (params.amount = this.amount);
                this.mobilePhone.trim() && (params.mobilePhone = this.mobilePhone.trim());
                this.wxAccount.trim() && (params.wxAccount = this.wxAccount.trim());
                this.genjinPeople.trim() && (params.responsible = this.genjinPeople.trim());
                this.nickname.trim() && (params.nickname = this.nickname.trim())
                this.ageStart.trim() && (params.ageStart = this.ageStart.trim())
                this.ageEnd.trim() && (params.ageEnd = this.ageEnd.trim())
                if(this.sex!=3){
                    params.sex = this.sex
                }

                this.channel1 && (params.primaryChannelName = this.channel1);
                this.channel2 && (params.twoLevelChannelName = this.channel2);

                if(this.isDisable==1){
                    params.isDisable=true;
                }else if(this.isDisable==2){
                ...
                if(this.isGag==1){
                    params.isGag=true;
                }else if(this.isGag==2){
                    params.isGag=false;
                }
                if(this.other==1){
                    params.pingRecord=true;
...
//                (this.isEnable || this.isEnable ===0) && (params.isEnable = this.isEnable);
            ...
            async MemberList(params) {
                ...
                let data = await getResponses("depression/member/getMemberDataStatisticsList.json",params)
                this.list = data.memberDataStatisticsList
                this.total = data.count
                this.countPennant = data.countPennant
...
//                })
            ...
            async getChannel(){
                const data = await getResponse('depression/member/getMemberDataStatisticsChannel.json');
                this.channelList = data.memberDataStatisticsChannelList;
            },
            ...
            channelChangeFn() {
                this.channel2List=[];
                if(this.channel1=="IOS"){
                    this.channel2List=this.channelList[0].twoLevelChannelNameList
                }else if(this.channel1=="Android"){
                    this.channel2List=this.channelList[1].twoLevelChannelNameList
                }else if(this.channel1=="后台"){
                    this.channel2List=this.channelList[2].twoLevelChannelNameList
                }else if(this.channel1=="微信"){
                    this.channel2List=this.channelList[3].twoLevelChannelNameList
                }else if(this.channel1=="web"){
                    this.channel2List=this.channelList[4].twoLevelChannelNameList
                }
                //      当一级渠道改变时获取并修改二级渠道列表
//                const data = await getResponses('depression/member/getMemberDataStatisticsChannel.json', `channalType=${this.channel1}`)
//                console.log(data)
//                if (data.code === 0){
...
<style scoped>
    ...
    .container-fluid .title#search select {
        width: 194px;
        margin: 0 6px 10px 0;
        height: 29px;
        display: inline-block;
