<template>
    <form enctype="multipart/form-data" v-if="type">
        <span style="margin-right:10px">{{name}}</span>
        <input name="key" type="hidden" value="111">
        <input type="hidden" name="imgPath" v-model="info.imgPath">
        <input type="file" class="span11 dn" id="photoAvatar{{eventName}}" @change="change">
        <a class="btn btn-info" style="margin: 0;" onclick="photoAvatar{{eventName}}.click()" v-if="btnState">上传</a>
        <a class="btn btn-large forbid" style="margin: 0;" v-else>上传</a>
    </form>
    <form enctype="multipart/form-data" v-else>
        <img :src=" curSrc " width="150" height="100" />
        <input type="hidden" name="imgPath" v-model="info.imgPath">
        <input type="file" class="span11 dn" id="photoAvatar{{eventName}}" @change="change">
        <a class="btn btn-info" onclick="photoAvatar{{eventName}}.click()" v-if="btnState">上传</a>
        <a class="btn btn-large forbid" v-else>上传</a>
    </form>
</template>
<script>
export default {
    props: ['eventName', 'defaultimg', 'type', "aa", "blob"],
